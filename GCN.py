...
def test(model, data, training_time_list, dataset_name, lossfunction_name):
    ...
    if os.path.exists('results_GCN.csv'):
        df.to_csv('results_GCN.csv', mode='a', header=False, index=False)
    else:
        df.to_csv('results_GCN.csv', mode='a', header=True, index=False)


# GCN
def run_GCN(dataset,dataset_name,lossfunction_name):
    batch_size = 256
    hidden_channels = 100
    size_gcn = 1
    lr = 0.01
