...
class GraphSAGE(nn.Module):
    def __init__(self, num_features, num_classes, hidden_channels=16, size_gnn=2, dropout=0.5):
        ...
        self.convs.append(SAGEConv(num_features, hidden_channels,aggr='mean'))
...
# GraphSAGE
def run_GraphSAGE(dataset,dataset_name,lossfunction_name):
    batch_size = 256
    hidden_channels = 100
    size_gnn = 1
    lr = 0.01
    ...
    model = GraphSAGE(num_features=data.num_features, num_classes=dataset.num_classes, hidden_channels=hidden_channels,
...
def run_GraphSAGE_hyp(dataset,lr,hidden_channels,size_gnn,weight_decay,epochs,dropout,batch_size):
    data = dataset[0]
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
    # Initialize the model and optimizer
    model = GraphSAGE(num_features=data.num_features, num_classes=dataset.num_classes, hidden_channels=hidden_channels,
